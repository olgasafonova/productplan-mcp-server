...
**Defending `pkg/productplan` as a reusable library boundary.** Rejected after checking what the server actually imports: only `AdaptiveRateLimiter`, `ParseAPIError`/`APIError`, and the `Require*` validators. When this was written, five of the package's modules (`retry.go`, `batch.go`, `health.go`, `requestid.go`, and the `registry.go` ToolBuilder) were tested but imported by nothing in `internal/` or `cmd/`. Since 24-09-2026 `batch.go` runs the `bulk_*_bars` tools, so four remain unwired, and the LRU cache the README still lists at `pkg/productplan/cache.go` was extracted to `github.com/olgasafonova/mcp-cache-go`. An article defending a boundary mostly made of unwired code would codify aspiration, not practice.
...
| 24-09-2026 | Amendment 1. Article VI now requires unknown tool arguments to be refused (closing its named gap). Articles I and II record their closed gaps (handler-map walk test, panic correlation ID). Article III adds `CacheStats` to the exhaustive exemption list. Articles I, VII, X, XII and XIII updated to 50 tools, go-sdk v1.8.0 `SetCacheable`, the refreshed `server.json`, the Go 1.26/1.27 test matrix and golangci-lint v2.13.2. |
