...
// milestones), one after another. The client exposes bars only together
// with their lanes (fetched concurrently), so the bars and lanes-for-bars
// calls go straight to the test server to keep the baseline sequential.
func sequentialComplete(ctx context.Context, c *api.Client, baseURL string, id api.RoadmapID) error {
	if _, err := c.GetRoadmap(ctx, id); err != nil {
		return err
	}
	for _, suffix := range []string{"/bars", "/lanes"} {
		if err := rawGet(ctx, baseURL+"/roadmaps/"+string(id)+suffix); err != nil {
			return err
		}
	}
...
// rawGet issues one GET and drains the body.
func rawGet(ctx context.Context, url string) error {
...
func BenchmarkGetRoadmapComplete(b *testing.B) {
	...
	b.Run("sequential_baseline", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			if err := sequentialComplete(ctx, c, srv.URL, "5"); err != nil {
				b.Fatal(err)
			}
		}
	})
